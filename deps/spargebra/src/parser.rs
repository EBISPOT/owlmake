...
impl ParserState {
    ...
    fn parse_iri(&self, iri: String) -> Result<Iri<String>, IriParseError> {
        let strict = if let Some(base_iri) = &self.base_iri {
            base_iri.resolve(&iri)
        } else {
            Iri::parse(iri.clone())
        };
        // owlmake: fall back to an UNCHECKED IRI rather than failing the parse.
        // Ontology repositories ship query files naming IRIs that RFC 3987 does not
        // admit, and those queries are expected to run. MONDO's
        // `qc-illegal-properties.sparql` names
        // `<http://www.geneontology.org/formats/oboInOwl#http://purl.obolibrary.org/obo/mondo#source>`,
        // whose second `#` makes the fragment invalid; rejecting it failed the whole
        // query, and a QC check that cannot run is a check that silently stops
        // guarding. The term is kept VERBATIM, so it still matches the identical
        // string on the data side, which om's RDF parser accepts on the same grounds.
        strict.or_else(|e| {
            if self.base_iri.is_some() {
                Err(e)
            } else {
                Ok(Iri::parse_unchecked(iri))
            }
        })
    }
