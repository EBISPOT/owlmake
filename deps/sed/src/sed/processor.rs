...
/// Append `text[a..b]`, where `a` and `b` are BYTE offsets from the regex engine.
///
/// A byte-oriented pattern matches — and, when it matches the empty string,
/// advances — one byte at a time, so the offsets are not always character
/// boundaries. sed works on bytes, so the span is appended as it stands rather
/// than panicking on the slice.
fn push_span(result: &mut String, text: &str, a: usize, b: usize) {
    match text.get(a..b) {
        Some(s) => result.push_str(s),
        None => result.push_str(&String::from_utf8_lossy(&text.as_bytes()[a..b])),
    }
}
...
fn substitute(
        ...
        (1, 0) => {
            ...
            match regex.find(pattern) {
                ...
                Ok(Some(m)) => {
                    ...
                    push_span(&mut result, text.unwrap(), last_end, m.start());
    ...
    if replaced {
        push_span(&mut result, text.unwrap(), last_end, text.unwrap().len());
